...
// Shared PCR log-row shape — the patient drawer's PCR automation-log tab reads
// the same builder so the two screens never disagree.
function rcdPcrLogRows() {
  const src = (typeof window!=="undefined" && window.RCD_ROWS) || [];
  return src.map((r,i)=>({
    id: r.id,
...
if (typeof window!=="undefined") window.rcdPcrLogRows = rcdPcrLogRows;
...
const RCD_PCR_COLS = [
  ...
  { key:"submissionType", label:"Submission Type", w:"130px", sort:true },
...
const RCD_PCR_FIELDS = [
  ...
  { key:"st",             label:"Start Time" },
  { key:"et",             label:"End Time" },
];
              ...
              {rows.length===0
                ...
                : rows.map(r=>(
                  <div key={r.ref} style={{display:"grid",gridTemplateColumns:grid,alignItems:"center",minHeight:52,padding:"6px 12px",border:"1px solid #e2e8f0",borderRadius:6,marginBottom:6,background:"#fff",boxShadow:"0 1px 2px rgba(15,23,42,0.03)",gap:6,position:"relative"}}>
                    ...
                    {bodyKeys.map(k=>(
                      <div key={k} title={r[k]} style={{font:"500 12px/16px Roboto",color:"#333333",display:"flex",alignItems:"center",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",minWidth:0,padding:"0 2px"}}>{r[k]==="—"?<span style={{color:"#cbd5e1"}}>—</span>:r[k]}</div>
                    ))}
                    <div style={{display:"flex",alignItems:"center",justifyContent:"center"}}><RCDLogATIcon kind={r.at} /></div>
                  </div>
                ))}
